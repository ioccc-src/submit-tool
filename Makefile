...
BIN_SRC= bin/genflaskkey.sh bin/ioccc_date.py bin/ioccc_passwd.py bin/set_slot_status.py \
	 ...
	 bin/mfile-ioccc.sh bin/acct_status.sh \
...
root_setup: ${INSTALL_UNDER_DOCROOT} ${PW} ${STATE} ${BIN_SRC} ${FLASHKEY} dist/${PKG_NAME}-${VERSION}-py3-none-any.whl
	...
	${CHMOD} 2770 ${DOCROOT}/save.users
	${RSYNC} -avzP save.users/ ${DOCROOT}/save.users
	${CHOWN} -R ${USER}:${GROUP} ${DOCROOT}/save.users
	${FIND} ${DOCROOT}/save.users -type d -print0 | ${XARGS} -0 ${CHMOD} -v 2750
	${FIND} ${DOCROOT}/save.users -type f -print0 | ${XARGS} -0 ${CHMOD} -v 0644
	${V} echo DEBUG =-= $@ end =-=
