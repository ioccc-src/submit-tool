#!/usr/bin/env bash
#
# jval.sh - write JSON values to stdout using the XPath for JSON model
#
# We form a wrapper for the `jval(1)`, a tool that allow the printing JSON values to stdout
# using the XPath for JSON model.
#
# NOTE: This code is based on the bin/jval-wrapper.sh from the IOCCC winner repo:
#
#   https://github.com/ioccc-src/winner/blob/master/bin/jval-wrapper.sh
#
# Unlike that IOCCC winner repo tool, this tool does not concern itself with topdir,
# nor bin tools, any git repo, nor does it change the current directory.
#
