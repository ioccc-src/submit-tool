...
export USAGE="usage: $0 [-h] [-v level] [-V] [-n] [-N] [-t topdir] [-T tmpdir]
	[-g gen_acct] [-r reg_email] [-p ioccc_passwd] email
...
# parse command line
#
while getopts :hv:VnNt:T:g:r:p: flag; do
  case "$flag" in
    h) echo "$USAGE" 1>&2
	exit 2
	;;
...
if [[ -z $NOOP ]]; then
    if [[ $V_FLAG -ge 1 ]]; then
	echo "$0: debug[1]: about to run: $GEN_ACCT_SH -p $IOCCC_PASSWD_PY $EMAIL > $TMP_NEW_ACCT_INFO" 1>&1
    fi
    "$GEN_ACCT_SH" -p "$IOCCC_PASSWD_PY" "$EMAIL" > "$TMP_NEW_ACCT_INFO"
    status="$?"
    if [[ $status -ne 0 ]]; then
    ...
    echo "$0: debug[1]: because of -n, did not run: $GEN_ACCT_SH -p $IOCCC_PASSWD_PY $EMAIL > $TMP_NEW_ACCT_INFO" 1>&2
fi
